...
EPathFindingResultState ANavigationVolumeActor::FindPathByVectors(APawn* findPawn, TArray<FVector>& resultRoute, FVector start, FVector end)
{
	return PathFindingComponent->FindPath(findPawn, resultRoute, start, end);
}

EPathFindingResultState ANavigationVolumeActor::FindPathByActors(APawn* findPawn, TArray<FVector>& resultRoute, AActor* start, AActor* end)
{
	return PathFindingComponent->FindPath(findPawn, resultRoute, start->GetActorLocation(), end->GetActorLocation());
}

EPathFindingResultState ANavigationVolumeActor::FindPathBySceneComponent(APawn* findPawn, TArray<FVector>& resultRoute, USceneComponent* start, USceneComponent* end)
{
	return PathFindingComponent->FindPath(findPawn, resultRoute, start->GetComponentLocation(), end->GetComponentLocation());
}

void ANavigationVolumeActor::DrawPathFromVector(FVector start, TArray<FVector> route, FColor pathColor, float duration, float pathThickness)
{
	PathFindingComponent->DrawPath(start, route, pathColor, duration, pathThickness);
}

void ANavigationVolumeActor::DrawPathFromActor(AActor* start, TArray<FVector> route, FColor pathColor, float duration, float pathThickness)
{
	PathFindingComponent->DrawPath(start->GetActorLocation(), route, pathColor, duration, pathThickness);
}

void ANavigationVolumeActor::DrawPathFromSceneComponent(USceneComponent* start, TArray<FVector> route, FColor pathColor, float duration, float pathThickness)
{
...
UWaypointComponent* ANavigationVolumeActor::CreateWaypoint(FVector location, FVector extent, FString name, USceneComponent* inParent)
{
...
void ANavigationVolumeActor::CreateOctree(UWaypointComponent* waypoint, int32 recursion, int32 recursionIndex, USceneComponent* inParent)
{
...
void ANavigationVolumeActor::CreatePaths(const TArray<UWaypointComponent*>& waypointList, TArray<UWaypointPathComponent*>& waypointPathList, FColor color, float thickness)
{
	TArray<FOverlapResult> overlapResults;
...
UWaypointPathComponent* ANavigationVolumeActor::CreateWaypointPath(UWaypointComponent* waypoint1, UWaypointComponent* waypoint2, FString pathName, FColor color, float thickness)
{
...
void ANavigationVolumeActor::DebugSettings(TArray<UWaypointComponent*>& waypointList, TArray<UWaypointPathComponent*>& waypointPathList, bool isUseWaypointCollisions, bool isVisiblePaths)
{
